...
namespace KevinPapst\TablerBundle\Command;
...
/**
 * Command used to download the configured tabler icons.
 * It will download all app icons, which will be stored in assets/icons/
 *
 * It also supports converting the previously used Fontawesome webfont configuration to UX icons (with SVG usage).
 * After conversion, you have top adapt your config as config/packages/tabler.yaml
 */
...
final class IconsCommand extends Command
{
    ...
    {
        $result = [];
        foreach ($this->tablerBundleIcons as $internal => $external) {
            if (str_contains($external, ':')) {
                $external = explode(':', $external);
                $result[$internal] = [$external[0] => $external[1]];
                continue;
            }
            foreach ($this->getPackageMappings() as $prefix => $package) {
                if (str_starts_with($external, $prefix)) {
                    $external = str_replace($prefix, '', $external);
                    $result[$internal] = [$package => $this->rename5To6($external)];
                }
            }
        }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        try {
            ...
            foreach ($this->getIconMapping() as $internalName => $packages) {
                foreach ($packages as $package => $name) {
                    ...
                    $options = ['names' => [$package . ':' . $name]];
                    if (Command::FAILURE === $command->run(new ArrayInput($options), $output)) {
                        $failures[$internalName] = [$package, $name];
                    }
                }
            }
        } catch (Exception $ex) {
            $io->error('Failed downloading icon: ' . $ex->getMessage());
        ...
        if (\count($failures) > 0) {
            $rows = [];
            foreach ($failures as $key => $values) {
                $rows[] = [
                    $key,
                    $values[0],
                    $values[1],
                    'bin/console ux:icons:search ' . $values[0] . ' ' . $values[1],
                ];
            }

            $io->error('Problems downloading the following files');
            $io->table(['Internal', 'Package', 'Name', 'Command'], $rows);
        } else {
            $io->success('Successful downloaded all icons');

            $convertedConfig = $this->getConvertedConfig();
            if (\count($convertedConfig) === 0) {
                $io->writeln('It seems your config is already up-to-date' . PHP_EOL);
            } else {
                $io->writeln('Now it is time to convert your configuration, in order to use UX icons.');
                $io->title('New configuration for config/packages/tabler.yaml');
                foreach ($this->getConvertedConfig() as $name => $item) {
                    $io->writeln($name . ': "' . $item . '"');
                }
            }
        }
     ...
     */
    private function rename5To6(string $name): string
    {
        return match ($name) {
            'ad' => 'rectangle-ad',
